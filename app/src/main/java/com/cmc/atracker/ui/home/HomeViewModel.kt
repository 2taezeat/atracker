...
import com.cmc.atracker.model.repository.RepositoryHome
...
class HomeViewModel : ViewModel() {
    ...
    val homeAddSelectedProgress : LiveData<ArrayList<HomeAddProgress>> =
        _homeAddSelectedProgress // HomeAddCalendarEventsAdapter 에 사용
    ...
    private val _homeDetailArrayList =
        MutableLiveData<ArrayList<HomeProgressItem>>().apply { // home Detail progress bar drawable 용
            value = arrayListOf()
        }
    ...
    private val _homeDetailContentForDisplay =
        MutableLiveData<ArrayList<HomeDetailItem>>().apply { // map 아님 주의
            arrayListOf<HomeDetailItem>()
        }
    val homeDetailContentForDisplay : LiveData<ArrayList<HomeDetailItem>> =
        _homeDetailContentForDisplay
    ...
    fun setZonedHomeAddProgress(position : Int?) {
        ...
        _homeAddSelectedStage.value!![position!!].event_at =
            zoneToLocal.toString() + 'Z' // time 값을 정확히 맞추기 위함.
    ...
    fun switch(mutableLiveData : MutableLiveData<Boolean?>) {
        mutableLiveData.value =
            if (mutableLiveData.value == null) true else !mutableLiveData.value!!
    }
    ...
    fun setHomeEdit() {
        ...
        _companyId.value =
            _homeApplyIdContent.value!!.company_id //_positionWord.value = _homeApplyIdContent.value!!.job_position
    ...
    fun stageContentParsing(
        ...
        val allParsedContentStringList =
            arrayListOf<String?>(null, null, null, null, null, null, null)
    ...
    fun getCompanyInfo(
        ...
        viewModelScope.launch {
            val apiResult = repositoryHome.companySearchPostCall(
                accessToken = App.prefs.getValue(BuildConfig.ACCESS_LOCAL_TOKEN)!!,
                companySearchRequest = CompanySearchRequest(title = searchWord, userDefined = true),
                page = page,
                size = size
            )
    ...
    fun addCompanyInfo() {
        viewModelScope.launch {
            val apiResult = repositoryHome.companyAddPostCall(
                accessToken = App.prefs.getValue(BuildConfig.ACCESS_LOCAL_TOKEN)!!,
    ...
    fun getStage() {
        viewModelScope.launch {
            val apiResult =
                repositoryHome.stageGetCall(accessToken = App.prefs.getValue(BuildConfig.ACCESS_LOCAL_TOKEN)!!)

            if (apiResult.code() == 200) {
                val getResult = apiResult.body()!!
                Log.d("getResult", "${getResult}")
                _homeAddStagesContent.value =
                    getResult //ApiExceptionUtil._apiExceptionFlag.value = Event(true)
    ...
    fun postApply() {
        ...
        viewModelScope.launch {
            val apiResult = repositoryHome.createApplyPostCall(
                accessToken = App.prefs.getValue(BuildConfig.ACCESS_LOCAL_TOKEN)!!,
    ...
    fun updateApply(applyId : Int) {
        val updateApplyRequest = UpdateApplyRequest(
            apply_id = applyId,
        ...
        viewModelScope.launch {
            val apiResult = repositoryHome.updateApplyPutCall(
                accessToken = App.prefs.getValue(BuildConfig.ACCESS_LOCAL_TOKEN)!!,
    ...
    fun deleteApply(deleteIds : Array<Int>) {
        viewModelScope.launch {
            val apiResult = repositoryHome.deleteApplyCall(
                accessToken = App.prefs.getValue(BuildConfig.ACCESS_LOCAL_TOKEN)!!,
                ids = deleteIds
            )
            ...
            if (apiResult.code() == 200) {
                ...
                for (apply in getResult) {
                    ...
                    for (stage in stageProgress) {
                        val stageStatus =
                            stage.status // status type => FAIL, IN_PROGRESS, NOT_STARTED, PASS 4가지 ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        viewModelScope.launch {
            ...
            if (apiResult.code() == 200) {
                ...
                for (stage in stageProgress) { // 단위는 stage 당 (서류 당, 1차 면접 당...)
                    ...
                    val stageStatus =
                        stage.status // String 상태 // status type => FAIL, IN_PROGRESS, NOT_STARTED, PASS 4가지, 서버에서 받아 올때는 String 임.
                    ...
                    _arrayListStageProgresse.value!!.add(
                        StageProgresse(
                    ...
                    for (sContent in stageContents) {
                        ...
                        val contentContentType =
                            sContent.content_type // FREE_FORM, NOT_DEFINED, OVERALL, QNA, 서버로 받아 올때는 string 임
                        val contentContent =
                            sContent.content // * contentContentType 에 따른 문자열 처리 해주어야함, // QNA = "{ \"q\": \"질문1\", \"a\": \"답변1\", \"f\": \"피드백\",}" /// FREE_FORM => "{ \"t\": \"자유 예시 타이틀\", \b\": \"자유 예시 컨텐츠\"}", OVERALL => "종합후기 예시 텍스트1"

                        val decidedContentContentType =
                            decideProgressItemBodyType(contentContentType)
                        val allParsedContentStringList =
                            stageContentParsing(contentContent, decidedContentContentType)
                        ...
                        when (contentContentType) {
                            ProgressItemBodyType.OVERALL.toString() -> {
                                forDetailDisplayArrayList.add(
                                    HomeDetailItem(
                                        progressType = stageOrder,
                                        progressName = stageTitle,
                                        itemType = ProgressItemBodyType.OVERALL,
                                        freeTitle = allParsedContentStringList[0],
                                        freeBody = allParsedContentStringList[1],
                                        totalReviewBody = allParsedContentStringList[2],
                                        questionBody = allParsedContentStringList[3],
                                        answerBody = allParsedContentStringList[4],
                                        qnaReviewBody = allParsedContentStringList[5],
                            ...
                            ProgressItemBodyType.QNA.toString() -> {
                                forDetailDisplayArrayList.add(
                                    HomeDetailItem(
                                        progressType = stageOrder,
                                        progressName = stageTitle,
                                        itemType = ProgressItemBodyType.QNA,
                                        freeTitle = allParsedContentStringList[0],
                                        freeBody = allParsedContentStringList[1],
                                        totalReviewBody = allParsedContentStringList[2],
                                        questionBody = allParsedContentStringList[3],
                            ...
                            ProgressItemBodyType.FREE_FORM.toString() -> {
                                forDetailDisplayArrayList.add(
                                    HomeDetailItem(
                                        progressType = stageOrder,
                                        progressName = stageTitle,
                                        itemType = ProgressItemBodyType.FREE_FORM,
                                        freeTitle = allParsedContentStringList[0],
                                        freeBody = allParsedContentStringList[1],
                                        totalReviewBody = allParsedContentStringList[2],
                                        questionBody = allParsedContentStringList[3],
    ...
    fun getMyApplyPfratio() { // getApplyDisplay 와 같은 이유로 일단 예외 처리 보류
        viewModelScope.launch {
            val apiResult =
                repositoryMyPage.myApplyPfratioGetCall(accessToken = App.prefs.getValue(BuildConfig.ACCESS_LOCAL_TOKEN)!!)
            if (apiResult.code() == 200) {
                val getResult = apiResult.body()!!
    ...
    fun isPassingUpdate(
        progressName : String,
        inProgressIsChecked : Boolean,
        failIsChecked : Boolean,
        ...
        _arrayListStageProgresse.value!!.forEach {
            it.new_contents.sortBy {
                ...
                when (it.content_type) {
                    ProgressItemBodyType.OVERALL.toString() -> order =
                        ProgressItemBodyType.OVERALL.ordinal
                    ProgressItemBodyType.QNA.toString() -> order =
                        ProgressItemBodyType.OVERALL.ordinal
                    ProgressItemBodyType.FREE_FORM.toString() -> order =
                        ProgressItemBodyType.OVERALL.ordinal
                    ProgressItemBodyType.NOT_DEFINED.toString() -> order =
                        ProgressItemBodyType.OVERALL.ordinal
                }
                order
            }
